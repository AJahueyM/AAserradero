...
export default function ReservationsPage() {
  ...
  return <Stack spacing={3}>
    ...
    <Paper sx={{ p: 2 }}><Stack direction="row" spacing={1.5} sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      <TextField label={t('reservations.searchLabel')} value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') handleSearch(); }} helperText={t('reservations.searchHint')} size="small" sx={{ flex: '1 1 320px', minWidth: 240 }} />
      <Button variant="contained" onClick={handleSearch} disabled={searchMutation.isPending || !searchTerm.trim()} sx={{ whiteSpace: 'nowrap' }}>{t('reservations.searchButton')}</Button>
      {canManage && <Button variant="contained" startIcon={<AddIcon />} onClick={() => setDialogState({ mode: 'create' })} sx={{ whiteSpace: 'nowrap', flexShrink: 0, px: 2.25 }}>{t('reservations.newReservation')}</Button>}
    </Stack></Paper>
...
function CalendarGrid({ month, rooms, reservations, onAvailableClick, onReservationClick }: { month: Date; rooms: RoomDto[]; reservations: ReservationSummaryResponse[]; onAvailableClick: (roomId: number, day: Date) => void; onReservationClick: (reservationId: number) => void }) {
  ...
  return <TableContainer component={Paper} sx={{ maxHeight: '72vh' }}><Table stickyHeader size="small" aria-label={t('reservations.title')}><TableHead><TableRow><TableCell sx={{ minWidth: 140, py: 0.75 }}>{t('reservations.roomColumn')}</TableCell>{days.map((day) => <TableCell key={day.toISOString()} align="center" sx={{ minWidth: 42, px: 0.25, py: 0.5 }}><Typography variant="caption" sx={{ lineHeight: 1 }}>{format(day, 'EEEEE')}</Typography><Typography variant="caption" sx={{ fontWeight: 700, lineHeight: 1.1 }}>{format(day, 'dd')}</Typography></TableCell>)}</TableRow></TableHead><TableBody>{activeRooms.map((room) => <TableRow key={room.id} hover><TableCell component="th" scope="row" sx={{ py: 0.75 }}><Typography variant="body2" sx={{ fontWeight: 700, lineHeight: 1.1 }}>{room.name}</Typography><Typography variant="caption" color="text.secondary" sx={{ lineHeight: 1 }}>{room.areaName} · {t('reservations.capacityShort', { count: room.capacity })}</Typography></TableCell>{days.map((day) => { const cell = getCalendarCell(room, day, reservationsByRoom.get(room.id) ?? [], t); return <TableCell key={`${room.id}-${day.toISOString()}`} align="center" sx={{ p: 0.25 }}><CalendarCellButton cell={cell} onClick={() => cell.reservation ? onReservationClick(cell.reservation.id) : onAvailableClick(room.id, day)} /></TableCell>; })}</TableRow>)}</TableBody></Table></TableContainer>;
}
function CalendarCellButton({ cell, onClick }: { cell: CalendarCell; onClick: () => void }) {
  return <Tooltip title={cell.reservation ? <ReservationTooltip reservation={cell.reservation} /> : cell.label} arrow><Box component="button" type="button" onClick={onClick} aria-label={cell.ariaLabel} sx={{ width: '100%', minHeight: 34, border: '1px solid', borderColor: cell.borderColor, borderRadius: 1, cursor: 'pointer', color: 'text.primary', background: cell.background, display: 'flex', alignItems: 'center', justifyContent: 'center', p: 0.25, '&:focus-visible': { outline: '3px solid', outlineColor: 'primary.main' } }}>{cell.icon}</Box></Tooltip>;
}
...
function ReservationDialog({ state, referenceData, canManage, onClose, onSaved }: { state: DialogState; referenceData: ReferenceData; canManage: boolean; onClose: () => void; onSaved: (reservationId: number) => void }) {
  ...
  return <Dialog open={open} onClose={() => void requestClose()} fullWidth maxWidth="lg"><DialogTitle>{reservationId ? t('reservations.reviewTitle', { id: reservationId }) : t('reservations.createTitle')}</DialogTitle><DialogContent dividers><Stack spacing={3}>
    ...
    {detail && <><Divider /><MovementsSection reservation={detail} referenceData={referenceData} form={movementForm} canManage={canManage} pending={pending} editorOpen={movementDialogOpen} onFormChange={setMovementForm} onCreate={() => { setMovementForm(createMovementForm(referenceData)); setMovementDialogOpen(true); }} onEdit={(movement) => { setMovementForm(formFromMovement(movement)); setMovementDialogOpen(true); }} onCloseEditor={() => { setMovementDialogOpen(false); setMovementForm(createMovementForm(referenceData)); }} onSubmit={() => movementMutation.mutate()} onDelete={(movementId) => void handleDeleteMovement(movementId)} onRecompute={() => recomputeMutation.mutate()} /><Divider /><NotificationSection options={notificationOptions} preview={preview} pending={pending} canManage={canManage} onOptionsChange={setNotificationOptions} onPreview={() => previewMutation.mutate()} onSend={() => sendMutation.mutate()} /></>}
...
function ReservationReview({ reservation, promotors }: { reservation: ReservationDetailResponse; promotors: UserLookupDto[] }) {
  ...
  return <Stack spacing={2}><Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', alignItems: 'center' }}><Chip label={t('reservations.reference', { id: reservation.id })} /><Chip icon={<SyncAltIcon />} label={t('reservations.currentStatus', { status: reservation.status.label })} /></Stack><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 2 }}><InfoCard title={t('reservations.client')}><Typography variant="h6" sx={{ lineHeight: 1.15 }}>{reservation.client.name}</Typography><Typography color="text.secondary">{reservation.client.cellphone || reservation.client.phone}</Typography></InfoCard><InfoCard title={t('reservations.bookingInfo')} icon={<HotelIcon />}><Typography>{t('reservations.stayDates', { entry: formatLocal(reservation.entryDate, 'dd/MM/yyyy HH:mm'), exit: formatLocal(reservation.exitDate, 'dd/MM/yyyy HH:mm') })}</Typography><Typography>{t('reservations.staySummary', { nights: reservation.nights, occupants })}</Typography><Typography>{t('reservations.promotor')}: {promotor}</Typography><Typography>{t('reservations.room')}: {reservation.room.name}</Typography></InfoCard><InfoCard title={t('reservations.financialSummary')}><Typography variant="h6" sx={{ fontWeight: 700 }}>{money(reservation.financialSummary.outstandingBalance)}</Typography><Typography color="text.secondary">{t('reservations.charges')}: {money(reservation.financialSummary.charges)}</Typography><Typography color="text.secondary">{t('reservations.payments')}: {money(reservation.financialSummary.payments)}</Typography></InfoCard></Box>{reservation.notes ? <InfoCard title={t('reservations.notes')}><Typography color="text.secondary">{reservation.notes}</Typography></InfoCard> : null}</Stack>;
}
function MovementsSection({ reservation, referenceData, form, canManage, pending, editorOpen, onFormChange, onCreate, onEdit, onCloseEditor, onSubmit, onDelete, onRecompute }: { reservation: ReservationDetailResponse; referenceData: ReferenceData; form: MovementFormState; canManage: boolean; pending: boolean; editorOpen: boolean; onFormChange: (form: MovementFormState) => void; onCreate: () => void; onEdit: (movement: MovementResponse) => void; onCloseEditor: () => void; onSubmit: () => void; onDelete: (movementId: number) => void; onRecompute: () => void }) {
...
function getCalendarCell(room: RoomDto, day: Date, reservations: ReservationSummaryResponse[], t: (key: string, params?: Record<string, unknown>) => string): CalendarCell {
  ...
  const occupying = reservations.find((reservation) => utcDayNumber(reservation.entryDate) <= dayNumber && dayNumber < utcDayNumber(reservation.exitDate));
  const starting = reservations.find((reservation) => utcDayNumber(reservation.entryDate) === dayNumber);
  const ending = reservations.find((reservation) => utcDayNumber(reservation.exitDate) === dayNumber);
  const starts = Boolean(starting);
  const ends = Boolean(ending);
  const reservation = occupying ?? starting ?? ending ?? null;
  const past = dayNumber < todayNumber;
  ...
  const base = reservation ? statusBackground(reservation.status.code) : past ? '#f5f5f5' : '#eef7ee';
  const neutral = past ? '#eeeeee' : '#e8f5e9';
  const startBase = starting ? statusBackground(starting.status.code) : base;
  const endBase = ending ? statusBackground(ending.status.code) : base;
  const background = past && !reservation
    ? 'repeating-linear-gradient(45deg,#f5f5f5,#f5f5f5 6px,#eeeeee 6px,#eeeeee 12px)'
    : starts && ends
      ? `linear-gradient(to right, ${endBase} 0 50%, ${startBase} 50% 100%)`
      : starts
        ? `linear-gradient(to right, ${neutral} 0 50%, ${startBase} 50% 100%)`
        : ends
          ? `linear-gradient(to right, ${endBase} 0 50%, ${neutral} 50% 100%)`
          : base;
...
function statusBackground(code: string) { switch (code) { case 'Pending': return '#ffe082'; case 'Partial': return '#90caf9'; case 'Paid': return '#81c784'; case 'Maintenance': return '#ce93d8'; case 'Courtesy': return '#80cbc4'; case 'Cancelled': return '#ef9a9a'; default: return '#cfd8dc'; } }
function statusBorder(code: string) { switch (code) { case 'Pending': return '#ef6c00'; case 'Partial': return '#1565c0'; case 'Paid': return '#2e7d32'; case 'Maintenance': return '#6a1b9a'; case 'Courtesy': return '#00695c'; case 'Cancelled': return '#b71c1c'; default: return '#546e7a'; } }
