...
function ReservationReview({ reservation, promotors }: { reservation: ReservationDetailResponse; promotors: UserLookupDto[] }) {
  ...
  return <Stack spacing={2}><Paper variant="outlined" sx={{ p: 2 }}><Stack spacing={1.5}><Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', alignItems: 'center' }}><Chip label={t('reservations.reference', { id: reservation.id })} /><Chip icon={<SyncAltIcon />} label={t('reservations.currentStatus', { status: reservation.status.label })} /></Stack><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr auto' }, gap: 1.5, alignItems: 'start' }}><Stack spacing={0.25}><Typography variant="h6" sx={{ lineHeight: 1.15 }}>{reservation.client.name}</Typography><Typography color="text.secondary">{reservation.client.cellphone || reservation.client.phone}</Typography><Typography>{reservation.room.name}</Typography><Typography>{t('reservations.stayDates', { entry: formatLocal(reservation.entryDate, 'dd/MM/yyyy HH:mm'), exit: formatLocal(reservation.exitDate, 'dd/MM/yyyy HH:mm') })}</Typography><Typography color="text.secondary">{t('reservations.staySummary', { nights: reservation.nights, occupants })}</Typography></Stack><Stack spacing={0.25} sx={{ textAlign: { xs: 'left', md: 'right' } }}><Typography variant="caption" color="text.secondary">{t('reservations.financialSummary')}</Typography><Typography variant="h6" sx={{ fontWeight: 700 }}>{money(reservation.financialSummary.outstandingBalance)}</Typography><Typography variant="caption" color="text.secondary">{t('reservations.charges')}: {money(reservation.financialSummary.charges)}</Typography><Typography variant="caption" color="text.secondary">{t('reservations.payments')}: {money(reservation.financialSummary.payments)}</Typography></Stack></Box></Stack></Paper><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 2 }}><InfoCard title={t('reservations.bookingInfo')} icon={<HotelIcon />}><Typography>{t('reservations.promotor')}: {promotor}</Typography><Typography>{t('reservations.room')}: {reservation.room.name}</Typography><Typography>{t('reservations.balance')}: {money(reservation.financialSummary.outstandingBalance)}</Typography></InfoCard>{reservation.notes ? <InfoCard title={t('reservations.notes')}><Typography color="text.secondary">{reservation.notes}</Typography></InfoCard> : null}</Box></Stack>;
}
