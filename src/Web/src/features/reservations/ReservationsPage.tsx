...
function CalendarGrid({ month, rooms, reservations, onAvailableClick, onReservationClick }: { month: Date; rooms: RoomDto[]; reservations: ReservationSummaryResponse[]; onAvailableClick: (roomId: number, day: Date) => void; onReservationClick: (reservationId: number) => void }) {
  ...
  return <TableContainer component={Paper} sx={{ maxHeight: '72vh' }}><Table stickyHeader size="small" aria-label={t('reservations.title')}><TableHead><TableRow><TableCell sx={{ minWidth: { xs: 90, lg: 120 }, py: 0.5, px: 0.5 }}>{t('reservations.roomColumn')}</TableCell>{days.map((day) => <TableCell key={day.toISOString()} align="center" sx={{ minWidth: { xs: 26, lg: 30 }, px: 0, py: 0.25 }}><Typography variant="caption" sx={{ lineHeight: 1 }}>{format(day, 'EEEEE')}</Typography><Typography variant="caption" sx={{ fontWeight: 700, lineHeight: 1 }}>{format(day, 'dd')}</Typography></TableCell>)}</TableRow></TableHead><TableBody>{activeRooms.map((room) => <TableRow key={room.id} hover><TableCell component="th" scope="row" sx={{ py: 0.5, px: 0.5 }}><Typography variant="body2" sx={{ fontWeight: 700, lineHeight: 1 }}>{room.name}</Typography><Typography variant="caption" color="text.secondary" sx={{ lineHeight: 1, display: { xs: 'none', lg: 'block' } }}>{room.areaName} · {t('reservations.capacityShort', { count: room.capacity })}</Typography></TableCell>{days.map((day) => { const cell = getCalendarCell(room, day, reservationsByRoom.get(room.id) ?? [], t); return <TableCell key={`${room.id}-${day.toISOString()}`} align="center" sx={{ p: 0.125 }}><CalendarCellButton cell={cell} onClick={() => cell.reservation ? onReservationClick(cell.reservation.id) : onAvailableClick(room.id, day)} /></TableCell>; })}</TableRow>)}</TableBody></Table></TableContainer>;
}
function CalendarCellButton({ cell, onClick }: { cell: CalendarCell; onClick: () => void }) {
  return <Tooltip title={cell.reservation ? <ReservationTooltip reservation={cell.reservation} /> : cell.label} arrow><Box component="button" type="button" onClick={onClick} aria-label={cell.ariaLabel} sx={{ width: '100%', minHeight: { xs: 28, lg: 30 }, border: '1px solid', borderColor: cell.borderColor, borderRadius: 1, cursor: 'pointer', color: 'text.primary', background: cell.background, display: 'flex', alignItems: 'center', justifyContent: 'center', p: 0, '& .MuiSvgIcon-root': { fontSize: 18 }, '&:focus-visible': { outline: '3px solid', outlineColor: 'primary.main' } }}>{cell.icon}</Box></Tooltip>;
}
